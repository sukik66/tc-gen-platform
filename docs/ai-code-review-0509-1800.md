...
```mermaid
pie title 判定能力覆盖
    "确定性判定 (grep_existence + grep_forbidden)" : 40
    "AST 结构判定 (ast_pattern → 暂降级为 grep)" : 10
    "LLM 语义判定 (llm_semantic)" : 35
...
gantt
    ...
    section 计划中
    阶段1.5 复测验证        :active, v15, after s15, 1d
    阶段2 ast-grep 规则库   : s2a, after v15, 3d
    阶段2 Few-shot 校准     : s2b, after v15, 2d
    阶段2 证据预采集        : s2c, after s2b, 2d
    阶段2 验证              : v2, after s2c, 1d
    阶段3 反馈闭环          : s3, after v2, 5d
```

#### 阶段 2：ast-grep 规则增强 + Few-shot 校准 + 证据预采集
...
├── docs\
│   └── ai-code-review-0509-1800.md  ← 本文档
├── data\
